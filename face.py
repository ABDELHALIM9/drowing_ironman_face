...
def draw_face(face_detiles, face_start):
    turtle.penup()
    turtle.goto(face_start)
    turtle.pendown()
    turtle.color("#e6b800")
    turtle.begin_fill()
    for i in range(len(face_detiles[0])):
        x, y = face_detiles[0][i]
        turtle.goto(x, y)

    for i in range(len(face_detiles[1])):
        x, y = face_detiles[1][i]
        turtle.goto(x, y)
